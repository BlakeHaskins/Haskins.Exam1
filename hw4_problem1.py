"""
Chat GPT and Wikipedia were consulted for proper
use of code, clarity, and formatting.
"""
...
# --- HW4 required changes: use quad + fsolve (and numpy for solver array handling) ---
import numpy as np
...
def F_tlnpdf(args):
    ...
    # --- HW4 change: replace Simpson with quad ---
    P, _err = quad(lambda x: tln_PDF((x, mu, sig, F_DMin, F_DMax)), D_Min, D, limit=200)
    return float(P)
...
def makeSample(args, N=100):
    ...
    # --- HW4 change: replace Secant with fsolve ---
    def solve_D(P):
        # initial guess inside the interval
        x0 = D_Min + P * (D_Max - D_Min)

        def eqn(D_arr):
            D = float(D_arr[0])
            return np.array([F_tlnpdf((ln_Mean, ln_sig, D_Min, D_Max, D, F_DMax, F_DMin)) - P], dtype=float)

        root = fsolve(eqn, np.array([x0], dtype=float), xtol=1e-10, maxfev=200)
        D = float(root[0])

        # clamp for small numerical drift
        if D < D_Min:
            D = D_Min
        if D > D_Max:
            D = D_Max
...
def getFDMaxFDMin(args):
    ...
    # --- HW4 change: replace Simpson with quad ---
    F_DMax, _err1 = quad(lambda x: ln_PDF((x, mean_ln, sig_ln)), 0.0, D_Max, limit=200)
    F_DMin, _err2 = quad(lambda x: ln_PDF((x, mean_ln, sig_ln)), 0.0, D_Min, limit=200)

    return (float(F_DMin), float(F_DMax))
...
def main():
    ...
    Assumptions:
    - Rocks are assumed spherical.
    - Pre-sieved diameters D follow a log-normal distribution with ln(D) ~ N(mu, sigma).
    - Post-sieve distribution is truncated to [Dmin, Dmax] based on screen apertures.
    - Random sampling from the truncated distribution is performed by inverse transform sampling:
        1) generate P ~ U(0,1)
        2) solve ∫_{Dmin}^{D} f_trunc(x) dx - P = 0 using fsolve
    - Numerical integration uses quad as required for HW4.

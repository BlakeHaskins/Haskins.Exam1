...
from hw4_problem1 import makeSample, sampleStats, getFDMaxFDMin
...
def main():
    '''
    Performs a one-sided t-test to determine if Supplier B produces
    statistically significantly smaller gravel than Supplier A.

    Supplier A uses a 1 in x 1 in large aperture screen.
    Supplier B uses a 7/8 in x 7/8 in large aperture screen.

    H0: mean_B >= mean_A
    H1: mean_B < mean_A

    Uses the truncated log-normal gravel sampling code from HW4
    and the t-distribution CDF from HW3b.
    '''

    print("Gravel Supplier Statistical Comparison\n")

    #region user inputs with defaults
    ...
    #region report each sample mean and variance
    print("\nSupplier A samples:")
    means_A = []
    vars_A = []
    for i, s in enumerate(samples_A):
        ...
        means_A.append(mean)
        vars_A.append(var)
        print(f"Sample {i + 1}: mean = {mean:.4f}, var = {var:.6f}")
    ...
    #region mean and variance of the sampling means
    ...
    print(f"\nSupplier A: mean of sampling means = {mean_of_means_A:.4f}")
    print(f"Supplier A: variance of sampling means = {var_of_means_A:.6f}")

    print(f"\nSupplier B: mean of sampling means = {mean_of_means_B:.4f}")
    print(f"Supplier B: variance of sampling means = {var_of_means_B:.6f}")
    #endregion

    #region one-sided t-test
    print("\n--- Hypothesis Test ---")
    print("H0: mean_B >= mean_A")
    print("H1: mean_B < mean_A")
    print(f"alpha = {alpha:.2f}")

    t_stat = (mean_of_means_A - mean_of_means_B) / math.sqrt(var_of_means_A / N_samples + var_of_means_B / N_samples)

    df_num = (var_of_means_A / N_samples + var_of_means_B / N_samples) ** 2
    df_den = ((var_of_means_A / N_samples) ** 2) / (N_samples - 1) + ((var_of_means_B / N_samples) ** 2) / (N_samples - 1)
    df = df_num / df_den
    ...
    if p_val < alpha:
        ...
        print("Supplier B produces statistically significantly smaller gravel than Supplier A.")
    else:
        print("\nConclusion: Fail to reject H0.")
        print("There is not enough evidence to say Supplier B produces smaller gravel than Supplier A.")
    #endregion
